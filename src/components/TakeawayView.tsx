...
import { ArrowLeft, Clock, ShoppingBag, Plus, Minus, CreditCard, CheckCircle2, ChevronRight, AlertCircle, Star, ChevronUp, ChevronDown, Menu as MenuIcon, UserCheck, Loader2 } from 'lucide-react';
...
export default function TakeawayView() {
  ...
  const [paymentMethod, setPaymentMethod] = useState<'online' | 'restaurant'>('online');
  ...
  if (!restaurant) {
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-4">
        ...
        <button onClick={() => navigate(-1)} className="mt-6 px-6 py-2 bg-brand text-white rounded-xl font-bold">Go Back</button>
      </div>
    );
  }
  ...
  const liveMenu = (restaurant.liveMenu || []).map((item: any) => ({
    ...item,
    category: item.isAvailable ? (item.category || 'Recommended') : 'Currently Unavailable for Order',
    isVeg: item.isVeg !== undefined ? item.isVeg : (item.name.toLowerCase().includes('chicken') || item.name.toLowerCase().includes('mutton') ? false : true),
  }));
  ...
  const handleConfirmOrder = async () => {
    if (paymentMethod === 'online') {
      ...
      const payableAmount = cartTotal + Math.round((restaurant.gstPercentage || 5) / 100 * cartTotal) + 20;
      ...
      try {
        ...
        if (data.error || !data.body || !data.body.txnToken) {
           alert("Payment Initialization failed via Paytm: " + (data.error || data.body?.resultInfo?.resultMsg || "Unknown error"));
          ...
          handler: {
            ...
            transactionStatus: function (data: any) {
               ...
               if (data && data.STATUS === 'TXN_SUCCESS') {
                  ...
                  alert("Payment was not successful. Status: " + (data.STATUS || "Failed"));
                  setIsProcessingPayment(false);
               }
  ...
  return (
    <div className="min-h-screen bg-slate-50 relative pb-24 md:pb-8">
              ...
              {liveMenu.length === 0 ? (
                ...
                <div className="space-y-6">
                  {Object.entries(groupedMenu).map(([cat, items]: [string, any]) => {
                    ...
                    return (
                      <div key={cat} id={`cat-${cat.replace(/\s+/g, '-')}`} className="bg-white scroll-mt-24">
                        <button 
                          onClick={() => toggleCategory(cat)}
                          className="w-full flex items-center justify-between py-6 px-4 md:px-6 bg-white"
                        >
                          <h2 className="text-xl md:text-2xl font-bold text-slate-900">{cat} ({items.length})</h2>
                          ...
                          {!isCollapsed && (
                            ...
                            >
                              <div className="divide-y divide-slate-100 border-b-8 border-slate-50 pb-8 px-4 md:px-6">
                                {items.map((item: any) => (
                                  <div key={item.id} className={cn("flex justify-between py-6 gap-4", !item.isAvailable && "opacity-60 grayscale")}>
                                    {/* Left: Info */}
                                    <div className="flex-1 flex flex-col justify-start max-w-[65%]">
                                      <div className="mb-2">
                                        <DietaryIcon isVeg={item.isVeg} />
                                      </div>
                                      <h3 className="font-bold text-slate-800 md:text-lg leading-tight">{item.name}</h3>
                                      <div className="font-medium text-slate-700 mt-1">₹{item.price}</div>
                                      {item.rating && (
                                        <div className="flex items-center text-sm font-bold mt-1.5">
                                          <Star className="text-green-600 fill-green-600 mr-1" size={14} />
                                          <span className="text-green-600">{item.rating}</span>
                                          <span className="text-slate-500 ml-1 font-medium text-xs">({item.ratingCount || 0})</span>
                                        </div>
                                      )}
                                      {item.description && (
                                        <p className="text-slate-500 text-sm mt-3 line-clamp-2 md:line-clamp-3 leading-relaxed">{item.description}</p>
                                      )}
                                    </div>
                                    ...
                                    <div className="relative w-32 md:w-36 shrink-0 flex flex-col items-center">
                                      {item.image ? (
                                        <img src={item.image} alt={item.name} className="w-full h-32 md:h-36 object-cover rounded-2xl shadow-sm" />
                                      ) : (
                                      ...
                                      {item.isAvailable && (
                                        <div className="absolute -bottom-4 w-28 md:w-32 z-10 drop-shadow-md">
                                          <div className={cn(
                                            ...
                                            cart[item.id] ? "text-brand" : "text-green-600"
                                          )}>
                                            {cart[item.id] ? (
                                              ...
                                              >
                                                ADD
                                                {!cart[item.id] && <Plus size={14} className="opacity-50" />}
                                              </button>
                                            )}
                                          </div>
                                        </div>
                     ...
                     <div className="space-y-6">
                       ...
                       <div className="border-t border-slate-100 pt-4 space-y-2 text-sm">
                         ...
                         <div className="flex justify-between text-slate-500">
                           ...
                           <span>₹{Math.round(cartTotal * ((restaurant.gstPercentage || 5) / 100))}</span>
                         </div>
                         <div className="flex justify-between text-slate-500">
                         ...
                         <div className="flex justify-between font-black text-slate-900 text-lg pt-2 mt-2 border-t border-slate-100">
                           <span>To Pay</span>
                           <span>₹{cartTotal + Math.round(cartTotal * ((restaurant.gstPercentage || 5) / 100)) + 20}</span>
                         </div>
                       </div>
               ...
               <div className="space-y-6">
                 <div>
                   ...
                   <div className="bg-slate-50 p-4 rounded-xl space-y-4">
                          ...
                          <div key={itemId} className="flex justify-between items-start gap-4">
                            <div className="flex items-start gap-2 flex-1">
                              ...
                              <div>
                                ...
                                <div className="font-medium text-slate-500 text-sm">₹{item.price}</div>
                              </div>
                            </div>
                            <div className="flex flex-col items-end gap-2 shrink-0">
                               <div className="flex items-center bg-white border border-slate-200 rounded-lg overflow-hidden h-8 shadow-sm">
                                 <button onClick={() => updateCart(itemId, false)} className="w-8 h-full flex items-center justify-center hover:bg-slate-50 text-brand"><Minus size={14}/></button>
                                 <span className="w-6 text-center text-sm font-bold text-slate-900">{quantity}</span>
                                 <button onClick={() => updateCart(itemId, true)} className="w-8 h-full flex items-center justify-center hover:bg-slate-50 text-brand"><Plus size={14}/></button>
                               </div>
                              <span className="font-bold text-slate-900 text-sm">₹{item.price * quantity}</span>
                            </div>
                          </div>
                        );
                     })}
                     ...
                     <div className="border-t border-slate-200 pt-4 space-y-2 text-sm">
                       ...
                       <div className="flex justify-between text-slate-600">
                         <span>GST ({(restaurant.gstPercentage || 5)}%)</span>
                         <span className="font-medium text-slate-900">₹{Math.round((restaurant.gstPercentage || 5) / 100 * cartTotal)}</span>
                       </div>
                       <div className="flex justify-between text-slate-600">
                         <span>Platform Fee</span>
                     ...
                     <div className="border-t border-slate-200 pt-4 flex justify-between font-black text-lg">
                       <span>To Pay</span>
                       <span className="text-brand">₹{cartTotal + Math.round((restaurant.gstPercentage || 5) / 100 * cartTotal) + 20}</span>
                     </div>
                   </div>
                 </div>
                 ...
                 {!user || !profile?.phone ? (
                   <div className="bg-brand/5 border border-brand/20 p-6 rounded-2xl text-center space-y-4">
                     ...
                     <div>
                       <h3 className="font-bold text-brand">{!user ? 'Login Required' : 'Contact Details Required'}</h3>
                       <p className="text-sm text-brand/80 mt-1">Please provide your details to place order</p>
                     </div>
                     {!user ? (
                         <button
                         ...
                         <div className="flex gap-2">
                           <input type="tel" value={phoneInput} onChange={e=>setPhoneInput(e.target.value)} placeholder="Enter Phone Number" className="flex-1 px-4 py-3 bg-white border border-brand/20 outline-none focus:border-brand rounded-xl font-medium" />
                           <button 
                             onClick={async () => {
                               if (phoneInput && phoneInput.length >= 10 && user.uid) {
                                  await updateDoc(doc(db, 'users', user.uid), { phone: phoneInput });
                               }
                             }}
                   ...
                   <div className="space-y-4">
                     <h3 className="font-bold text-slate-900 mb-3 text-sm uppercase tracking-widest">Payment Method</h3>
                     <div className="flex flex-col sm:flex-row gap-3">
                        ...
                        <label className="flex-1 flex items-center gap-3 p-4 border border-slate-200 rounded-xl cursor-pointer hover:border-brand/50 transition-colors has-[:checked]:border-brand has-[:checked]:bg-brand/5">
                          <input type="radio" name="payment_method" value="restaurant" checked={paymentMethod === 'restaurant'} onChange={() => setPaymentMethod('restaurant')} className="hidden" />
                          <div className="w-5 h-5 rounded-full border-2 border-slate-300 flex items-center justify-center">
                            <div className="w-2.5 h-2.5 rounded-full bg-brand opacity-0 transition-opacity"></div>
                          </div>
                          <span className="font-bold text-slate-900">Pay at Restaurant</span>
                        </label>
                     </div>
        ...
        {step === 'success' && (
          <div className="max-w-md mx-auto text-center py-12">
            ...
            <p className="text-slate-500 mb-8">
              Your takeaway order has been placed successfully. The restaurant will notify you when it's ready for pickup.
              {paymentMethod === 'restaurant' && " Please pay at the restaurant upon collection."}
            </p>
               ...
               <div className="flex justify-between items-center pb-4 border-b border-slate-100 mb-4">
                 ...
                 <div className="text-right">
                   {paymentMethod === 'restaurant' ? (
                     <>
                       <div className="text-sm text-amber-600 font-bold">Payment Pending</div>
                       <div className="font-bold text-slate-900">₹{cartTotal + Math.round(cartTotal * ((restaurant.gstPercentage || 5) / 100)) + 20}</div>
                     </>
                   ) : (
                     <>
                       <div className="text-sm text-slate-500">Total Paid</div>
                       <div className="font-bold text-brand">₹{cartTotal + Math.round(cartTotal * ((restaurant.gstPercentage || 5) / 100)) + 20}</div>
                     </>
                   )}
                 </div>
               </div>
      ...
      <AnimatePresence>
        {step === 'menu' && (
          <div className={cn("md:hidden fixed right-6 z-[55] flex flex-col items-end gap-3 transition-all duration-300", cartItemsCount > 0 ? "bottom-24" : "bottom-6")}>
            {Object.keys(groupedMenu).length > 0 && (
              <button 
                onClick={() => {
                  const el = document.getElementById(`cat-${Object.keys(groupedMenu)[0].replace(/\s+/g, '-')}`);
        ...
        {step === 'menu' && cartItemsCount > 0 && (
          ...
          >
             <div className="flex items-center justify-between mx-auto max-w-sm">
               <div>
                 <div className="font-bold text-sm">{cartItemsCount} item{cartItemsCount > 1 ? 's' : ''} added</div>
               </div>
               <button 
                 onClick={handleCheckout}
