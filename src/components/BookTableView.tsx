...
export default function BookTableView() {
  ...
  const dates = useMemo(() => {
    ...
    return allDates.filter(
      (date) => !restaurant.blackoutDates?.includes(format(date, "yyyy-MM-dd")),
    );
  }, [restaurant]);
  ...
  if (bookingSuccess) {
    return (
      <div className="min-h-screen bg-slate-50 pt-4 px-4 md:pt-20 pb-12 flex flex-col items-center">
        <div className="max-w-md w-full bg-white rounded-3xl p-6 md:p-8 shadow-vibrant text-center border border-slate-100 mt-4 md:mt-0">
            ...
            <div className="space-y-4">
              {slotData.categories.map((cat) => {
                ...
                return (
                  <div
                    key={cat.id}
                    className="border border-slate-200 rounded-[20px] overflow-hidden bg-white"
                  >
                    <button
                      onClick={() =>
